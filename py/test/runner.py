...
class KanyerestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KanyerestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KanyerestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KanyerestTestRunner.getenv("KANYEREST_TEST_LIVE")
        override = KanyerestTestRunner.getenv("KANYEREST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KanyerestTestRunner.getenv(key)
        ...
        explain = KanyerestTestRunner.getenv("KANYEREST_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["KANYEREST_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if KanyerestTestRunner._test_control is not None:
            return KanyerestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KanyerestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KanyerestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KanyerestTestRunner.load_test_control()
...
def load_env_local():
    KanyerestTestRunner.load_env_local()


def env_override(m):
    return KanyerestTestRunner.env_override(m)


def entity_data(v):
    return KanyerestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KanyerestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KanyerestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KanyerestTestRunner.load_test_control()


def live_client_options():
    return KanyerestTestRunner.live_client_options()


def live_delay_ms():
    return KanyerestTestRunner.live_delay_ms()
