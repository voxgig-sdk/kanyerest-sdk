...
class KanyerestSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = KanyerestHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = KanyerestHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!KanyerestFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, KanyerestFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return KanyerestUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = KanyerestHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = KanyerestHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = KanyerestHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new KanyerestError($op . "_allow",
                "KanyerestSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = KanyerestHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new KanyerestError("graphql_error",
                "KanyerestSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetRandomQuote($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_random_quote === null) {
                $this->_get_random_quote = new GetRandomQuoteEntity($this, null);
            }
            return $this->_get_random_quote;
        }
        return new GetRandomQuoteEntity($this, $data);
    }
