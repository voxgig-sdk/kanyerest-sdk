...
class FtCtrl extends KanyerestControl
{
...
class FtClient
{
    ...
    private KanyerestUtility $utility;

    public function __construct(KanyerestUtility $utility, array $options)
    {
    ...
    public function get_utility(): KanyerestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KanyerestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KanyerestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KanyerestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KanyerestConfig::make_feature($name);
    ...
    private static function buildUrl(KanyerestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KanyerestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KanyerestResponse($fetched);
        }
    ...
    private function fail(KanyerestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KanyerestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
