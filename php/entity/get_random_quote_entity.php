...
// Kanyerest SDK GetRandomQuote entity
...
class GetRandomQuoteEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetRandomQuoteEntity($this->_client, $opts);
    }

    /**
     * @param GetRandomQuote|array $args GetRandomQuote data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = KanyerestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetRandomQuote|array The current GetRandomQuote data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetRandomQuote fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = KanyerestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetRandomQuote fields).
     */
    ...
    /**
     * Load a single GetRandomQuote.
     *
     * @param GetRandomQuoteLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetRandomQuoteLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetRandomQuote|array The loaded GetRandomQuote as an assoc-array at the
     *   SDK boundary; throws KanyerestError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = KanyerestHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
