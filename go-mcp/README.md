...
[MCP](https://modelcontextprotocol.io) server exposing the Kanyerest SDK as
two agent tools — `kanyerest_list` and `kanyerest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kanyerest-mcp)
make build

# 2. Provide credentials via the environment
export KANYEREST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kanyerest \
  -- /absolute/path/to/kanyerest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kanyerest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kanyerest_list: first page of records
{ "entity": "get_random_quote" }
{ "entity": "get_random_quote", "query": { } }

// kanyerest_load: one record by id
{ "entity": "get_random_quote", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kanyerest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kanyerest \
     -- "$PWD"/dist/*/kanyerest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kanyerest_list` and `kanyerest_load` tools now appear
   in new sessions. Ask the agent to *"list get_random_quote using kanyerest"*
   and it calls `kanyerest_list` with `{"entity":"get_random_quote"}`.
...
```sh
export KANYEREST_APIKEY=sk_live_xxx            # API key
export KANYEREST_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `kanyerest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kanyerest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KANYEREST_APIKEY` | API key sent with every request. |
| `KANYEREST_BASE` | Optional override of the API base URL. |
...
```sh
./kanyerest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kanyerest_load","arguments":{"entity":"get_random_quote","query":{"id":1}}}}'
```
