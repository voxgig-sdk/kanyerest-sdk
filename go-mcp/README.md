...
MCP server exposing the Kanyerest SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `kanyerest_list` | `entity`, optional `query` map | First page of records as JSON |
| `kanyerest_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `kanyerest_list` and
`kanyerest_load` tools become available in new sessions.
