...
import (
	...
	omni "github.com/voxgig-sdk/kanyerest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KanyerestError); is {
				out := map[string]any{
					"name":    "KanyerestError",
...
func omniCtx(arg any, client *sdk.KanyerestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KanyerestSDK); is {
			client = live
