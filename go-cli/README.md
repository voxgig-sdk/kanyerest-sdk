...
boru-driven command-line client **and** interactive REPL for the Kanyerest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kanyerest-cli)
make build
...
./kanyerest-cli load 1 get_random_quote            # {id:1} shorthand
./kanyerest-cli load '{id:1}' get_random_quote       # explicit match map

# 5. Override the API base URL for a single call
KANYEREST_BASE=https://api.example.com ./kanyerest-cli load 1 get_random_quote

# 6. No arguments -> interactive REPL
./kanyerest-cli
kanyerest> load 1 get_random_quote
kanyerest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kanyerest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kanyerest-cli load 1 get_random_quote
   ```
...
```sh
./kanyerest-cli load 1 get_random_quote          # scalar shorthand for {id:1}
./kanyerest-cli load '{id:1}' get_random_quote     # explicit match map
```
...
./kanyerest-cli load 1 get_random_quote
```
...
Run with no arguments to open a REPL (prompt `kanyerest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_quote`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kanyerest-cli`. |
...
(`./kanyerest-cli <expr>`) and interactively (the REPL), and expressions compose the
