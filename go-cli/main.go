// Command Kanyerest-cli is a CLI + REPL for the Kanyerest SDK,
...
const prompt = "kanyerest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_random_quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KANYEREST_APIKEY (api key), KANYEREST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KANYEREST_APIKEY carries the API key and
	// KANYEREST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KANYEREST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KANYEREST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKanyerestSDK(opts)
