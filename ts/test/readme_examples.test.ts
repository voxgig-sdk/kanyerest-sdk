...
//     (\`new KanyerestSDK(...)\` / \`KanyerestSDK.test(...)\` become
//     \`KanyerestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_random_quote":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KanyerestSDK(...)\` and any
// \`KanyerestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KanyerestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KanyerestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
