...
describe('GetRandomQuoteDirect', async () => {
  ...
  test('direct-load-get_random_quote', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_random_quote', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KANYEREST_TEST_GET_RANDOM_QUOTE_ENTID']
