...
Learn more about kanye.rest at [kanye.rest](https://kanye.rest).

This is an unofficial SDK for the kanye.rest public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — GetRandomQuote — that you
...
const getrandomquote = await client.GetRandomQuote().load()
// getrandomquote is the GetRandomQuote entity, populated with mock data
...
| --- | --- | --- |
| **GetRandomQuote** | The GetRandomQuote entity (load). | `/` |
...
// Load a specific getrandomquote (returns the ENTITY; call data_get() for the record; throws on error)
$getrandomquote = $client->GetRandomQuote()->load();
...
# Load a specific getrandomquote (returns the ENTITY; call data_get for the record)
getrandomquote = client.GetRandomQuote.load()
puts getrandomquote
```
...
- Upstream API: [https://kanye.rest/](https://kanye.rest/)
