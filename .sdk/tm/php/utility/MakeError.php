...
// Kanyerest SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class KanyerestMakeError
{
    public static function call(?KanyerestContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KanyerestContext([], null);
        }
        $op = $ctx->op ?? new KanyerestOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KanyerestResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KanyerestError) ? $err->msg : (string)$err;
        $msg = "KanyerestSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new KanyerestError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof KanyerestError) {
