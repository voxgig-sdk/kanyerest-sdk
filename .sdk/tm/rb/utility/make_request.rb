...
module KanyerestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KanyerestResponse.new({})
    ...
    if fetch_err
      ...
      response = KanyerestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KanyerestResponse.new(fetched)
    else
