...
require_relative 'Kanyerest_types'
...
class KanyerestSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KanyerestHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KanyerestHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KanyerestFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KanyerestUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KanyerestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KanyerestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KanyerestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = KanyerestHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KanyerestError.new(
        "graphql_error", "KanyerestSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
